<template>
  <div>
    ...
    <main role="main" class="container">
      <div class="row">
        <v-broadside
          class="col-md-2 broadside"
          :uid="uid"
          ref="broadSide"
          :hotArticles="hotArticles"
          :newArticles="newArticles"
          @change="mainShowChange"
          @mainShow="detailChange"
        ></v-broadside>
        <v-main
          class="mt-1 col-md-10 main-container"
          :uid="uid"
          @articleInfoChange="articleInfoChange"
          v-bind:class="{ mainHidden }"
        ></v-main>
      </div>
    </main>
...
export default {
  ...
  methods: {
    ...
    async getHotArticles() {
      try {
        const result = await this.$getAjax(
          `/myapi/articles/list/hot?authorUid=${this.uid}`
        );
    ...
    async getNewArticles() {
      try {
        const result = await this.$getAjax(
          `/myapi/articles/list/new?createdBy=${this.uid}`
        );
